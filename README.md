...
This only handles frame-by-frame drawn animation and symbol instances. Tweens (motion, shape, classic) aren't read or converted. Support for tweening is planned for a future version.
...
The top folder is whatever you picked, `c114_col04` is the scene folder named after the FLA, `v001` is the version folder, and `symbols` holds each unique symbol, exported once, regardless of how many instances use it.
...
c114_col04_v001/
  ...
  Dad_box (footage)
  dad_walk/
    dad_walk_box (footage)
    dad_walk_dad (footage)
  dad_walk (comp)
c114_col04_v001 (comp)
```

The drawn layers (`Amy`, `Amy_righthand`, `Dad`, `Dad_box`) are footage, and so is each layer inside the symbol itself (`dad_walk_box`, `dad_walk_dad`, inside the `dad_walk` folder, one for each of `dad_walk`'s own internal layers). `dad_walk` is the symbol comp, and `c114_col04_v001` is the master comp. Symbol comps and their footage folders share the same name for easy matching.
...
- First and last frame: sets the playback range. When only first frame is set, playback starts from there, for loops the full symbol still cycles, for play once it plays from that frame to the end. When both are set, only that range plays.
...
- Tweens (motion, shape, classic): not read or converted. Planned for a future version
