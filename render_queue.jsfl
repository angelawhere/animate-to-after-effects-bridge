// render_queue.jsfl
// Batch exports multiple FLA files using animate_export.jsfl. Keep both files in the same folder.
// If USE_DEFAULT_FOLDER is set in animate_export.jsfl, that folder is used for all scenes.
// Otherwise you are prompted once and the location is remembered for next time.
...
if (!FLfile.exists(exportScriptURI)) {
    ...
    // prevents animate_export.jsfl from auto-running main() on load
