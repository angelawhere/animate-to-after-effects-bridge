// animate_export.jsfl
// Exports keyframe timing and PNG sequences from Adobe Animate for import into After Effects via ae_import.jsx.
// Exports one PNG per keyframe per visible layer (drawn and symbol layers), and one scene JSON with all timing data.

var VERSION = "1.0";

var TEST_MODE = false; // dry run, logs what would be exported without writing files

var USE_DEFAULT_FOLDER = false;
var DEFAULT_FOLDER = "C:/users/you/documents/project/output/"; // used when USE_DEFAULT_FOLDER is true
...
function main(docPath, outputFolder) {
    ...
    // first pass collects unique symbols so each is exported once, regardless of instance count
    var symbolMap = {};
    var warnings = [];
    ...
    // second pass exports drawn layers and records symbol instance data
    var layersData = [];
    for (var i = 0; i < layers.length; i++) {
        var layer = layers[i];
        if (!originalVisibility[i] || layer.layerType !== "normal") continue;
    ...
    log("Export complete (v" + VERSION + "): " + layersData.length + " layer(s), " + Object.keys(symbolsData).length + " symbol(s)");
...
// shifts content to stage center during export to avoid clipping at the canvas edge, compensated in AE via compAnchorX/Y
...
// drawn frames export PNGs from the main timeline; symbol frames record instance data with no PNG export
// a layer may produce multiple aeLayers if it has blank gaps or mixes drawn frames and symbols
function exportLayer(doc, timeline, layer, layerIndex, versionFolder, fps, warnings) {
    ...
    // isolate this layer for a clean PNG export
    ...
    var layerFolderCreated = false; // only create the folder once a drawn frame is actually found
        ...
        if (isSymbol) {
            ...
            for (var e = 0; e < frame.elements.length; e++) {
                ...
                if (playMode === "single frame") {
                    ...
                    instanceLastFrame = -1;
                    layerWarnings.push("Frame " + f + ": reverse play once is not supported, exported as forward play once instead. Rearrange manually in AE.");
                } else if (playMode === "loop reverse") {
                    playType = "loop";
                    instanceFirstFrame = 0;
                    instanceLastFrame = -1;
                    layerWarnings.push("Frame " + f + ": reverse loop is not supported, exported as forward loop instead. Rearrange manually in AE.");
                } else {
                    playType = "loop";
                    if (instanceLastFrame !== -1 && instanceLastFrame <= instanceFirstFrame) {
                        layerWarnings.push("Frame " + f + ": last frame (" + (instanceLastFrame + 1) + ") must be greater than first frame (" + (instanceFirstFrame + 1) + "), last frame ignored.");
...
function getFrameType(frame) {
    ...
    if (hasShape && hasSymbol) return "mixed";
...
// only auto-runs when not called from render_queue.jsfl
